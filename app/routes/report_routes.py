    ...
    try:
        ...
        if not user.is_premium() and not user.is_admin:
            return jsonify({
                ...
                'message': 'PDF export is a premium feature. Please upgrade to access this feature.'
            }), 403
...
def share_report(scan_id):
    ...
    try:
        ...
        if not user.is_premium() and not user.is_admin:
            return jsonify({
                'error': 'premium_required',
                'message': 'Report sharing is a premium feature. Please upgrade to access this feature.'
            }), 403
