    ...
    try:
        ...
        if not user.can_scan():
            return jsonify({
                ...
                'message': 'You have reached your daily scan limit. Upgrade to premium for unlimited scans.'
            }), 429
        ...
        if not current_app.config.get('TESTING'):
            ...
            worker = threading.Thread(
                target=run_scan,
                args=(scan.id, app, remote_addr, auth_cookie),
                daemon=True
            )
            worker.start()
...
def run_scan(scan_id, app=None, remote_addr=None, auth_cookie=None):
