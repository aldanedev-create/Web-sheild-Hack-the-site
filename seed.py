...
def seed_subscription(admin_user):
    """Create a subscription record for the admin user."""

    # Check if subscription exists
    existing = Subscription.query.filter_by(user_id=admin_user.id).first()
    if existing:
    ...
    db.session.commit()
    print("Admin subscription created")
